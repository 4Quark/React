...
export interface IFormCard {id: number, name: string, date: string, select: string, switcher: string, skills: string, file: string, checkbox: boolean};
...
class MyForm extends React.Component<formProps, formState> {
    ...
    handleForm = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (this.fileInput.current) if (this.fileInput.current.files) this.setState({file: this.fileInput.current.files[0]?.name});
        
        if (this.nameInput.current && this.dateInput.current && this.selectInput.current && this.checkboxInput.current) {
            const lang = [];
            ...
            this.cards.push({
                ...
                skills: lang.join(', '),
                file: this.fileInput?.current?.files?URL.createObjectURL(this.fileInput.current.files[0]): '', 
    ...
    removeCard = (id: number) => {
        ...
        this.cards.forEach((el, i) => {if (el.id == id) this.cards.splice(i, 1)});
        console.log(this.cards);
    }
        ...
        return (
            <div className='form_content'>
                ...
                <div className='form_cards_content'>
                    {this.cards.length !== 0
                        ? this.cards?.map((card, i) => <FormCard key={i} remove={this.removeCard} id={card.id} name={card.name} date={card.date} select={card.select} switcher={card.switcher} skills={card.skills} file={card.file} checkbox={card.checkbox} />)
                        : <h2>Nothing found </h2>
                    }
                </div>
            </div>
